...
import { decidePurchaseOrderAction } from "@/app/(admin)/erp/actions";

/** Inline approve/reject controls for a 'requested' purchase order (D-014). Mirrors
 * allocations' DecisionButtons. */
export function PoDecisionButtons({ poId, tenantId }: { poId: string; tenantId: string }) {
  ...
  async function submit(action: "approve" | "reject") {
    ...
    const result = await decidePurchaseOrderAction(poId, {
      tenant_id: tenantId,
      action,
      actor: actor.trim(),
    });
