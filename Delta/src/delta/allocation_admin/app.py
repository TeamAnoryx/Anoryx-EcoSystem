...
D-022 subscriptions, D-023 investment allocation, D-024 micro-transaction execution,
D-025 multi-bank aggregation).
...
personal-finance, subscriptions, investments, micro-transactions, bank-aggregation)
plus a ``/health`` probe. One app/port for the whole admin console (D-008/.../D-025
add routes to the D-007 app rather than standing up a second process — same
operators, same auth, same trust boundary).
...
from ..bank_aggregation.router import router as bank_aggregation_router
...
from ..micro_transactions.router import router as micro_transactions_router
...
def create_app() -> FastAPI:
    ...
    app.include_router(bank_aggregation_router)
    return app
