...
async def get_pipeline_summary(session: AsyncSession, *, currency: str) -> PipelineSummary:
    ...
    every other Delta rollup's single-currency discipline). A deal with a NULL
    value/currency (an unqualified early-stage lead, per D-013's own pairing
    discipline) contributes to `open_deal_count` but not to the summed value.
    """
    client_count_stmt = select(func.count()).select_from(clients)
    client_count = (await session.execute(client_count_stmt)).scalar_one()

    open_deal_count_stmt = select(func.count()).where(deals.c.stage.notin_(_TERMINAL_STAGES))
