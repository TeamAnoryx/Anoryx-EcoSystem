"""Fixtures for the D-018 invoicing DB suite. Mirrors ``tests/erp/conftest.py``/
``tests/capacity/conftest.py`` exactly (same db_required/ensure_schema_at_head/
provision_app_role/_truncate/_reset_delta_engines/admin_token/app/client/auth_headers
shape).

D-018 depends on D-014 (vendors/purchase_orders) and, for the milestone-linked path,
D-015 (tasks) — ``delta.erp.store``/``delta.erp.service`` and ``delta.pm.store``/
``delta.pm.service`` are called directly (then committed) to seed prerequisite rows,
mirroring ``tests/capacity/conftest.py``'s identical cross-package seeding precedent.
"""
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.invoice_payments, delta.invoices, "
                    "delta.task_dependencies, delta.tasks, delta.sprints, "
...
async def seed_approved_po(
    ...
    Uses ``delta.erp.store`` directly (D-014's own real write path) then commits — the
    same cross-package seeding precedent ``tests/capacity/conftest.py`` establishes for
    D-015 tasks.
    """
...
async def seed_done_task(session, *, tenant_id: str) -> str:
    """Create a D-015 task and move it to 'done'. Returns task_id."""
    ...
    await pm_store.update_task_status(session, task_id=task.task_id, status="done", now=now)
    await session.commit()
    return task.task_id
