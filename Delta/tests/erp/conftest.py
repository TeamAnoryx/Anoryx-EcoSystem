"""Fixtures for the D-014 ERP DB suite. Mirrors ``tests/crm/conftest.py`` exactly (same
db_required/ensure_schema_at_head/provision_app_role/_truncate/_reset_delta_engines/
admin_token/app/client/auth_headers shape).

D-014 owns its own tables (migration 0008) — there is no lower-level "ingest" primitive
to seed through. ``delta.erp.store`` (called directly, then committed) IS the real,
only application path for writing a vendor/asset/purchase_order row.
"""
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text("TRUNCATE delta.purchase_orders, delta.assets, delta.vendors CASCADE")
            )
    finally:
