"""Fixtures for the D-025 bank-aggregation DB suite. Mirrors
``tests/micro_transactions/conftest.py`` exactly (same db_required/
ensure_schema_at_head/provision_app_role/_truncate/_reset_delta_engines/admin_token/
app/client/auth_headers shape).

D-025 owns ``linked_institutions``/``aggregation_sync_runs``/
``aggregation_ingested_references`` (migration 0018) and writes into D-021's
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.aggregation_ingested_references, "
                    "delta.aggregation_sync_runs, delta.linked_institutions, "
