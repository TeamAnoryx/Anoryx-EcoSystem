"""Fixtures for the D-017 RBAC DB suite. Mirrors ``tests/capacity/conftest.py``
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.task_dependencies, delta.tasks, delta.sprints, "
                    "delta.teams, delta.access_tokens CASCADE"
                )
            )
    finally:
