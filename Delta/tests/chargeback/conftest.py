"""Fixtures for the D-012 chargeback/anomaly DB suite.

D-012 adds NO migration (pure read aggregates over the existing D-003
``ledger_entries`` table, same as D-008) — this harness only needs the schema at
whatever head D-003..D-011 already established, plus a way to seed real ledger rows
via the real D-004 posting path (never hand-inserted rows). Mirrors
``tests/dashboards/conftest.py`` exactly.
"""
