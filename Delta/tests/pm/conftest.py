"""Fixtures for the D-015 project-management DB suite. Mirrors ``tests/erp/conftest.py``
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text("TRUNCATE delta.task_dependencies, delta.tasks, delta.sprints CASCADE")
            )
    finally:
