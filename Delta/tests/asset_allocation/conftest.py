"""Fixtures for the D-023 asset-allocation DB suite. Mirrors ``tests/subscriptions/
...
D-023 owns its own table (migration 0016), plus D-021's ``personal_accounts``/
``personal_transactions`` (an allocation recommendation always targets an existing
personal account, and its micro-investment figure is derived from recorded
transactions) — ``delta.asset_allocation.store``/``delta.personal_finance.store``
(called directly, then committed) ARE the real, only application paths for writing
these rows.
"""
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.personal_allocation_recommendations, "
                    "delta.personal_transactions, delta.personal_accounts CASCADE"
                )
            )
    finally:
