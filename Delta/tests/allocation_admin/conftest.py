"""Fixtures for the D-007 allocation-admin DB suite.

Mirrors the proven D-005/D-006 DB harness (tests/budget_engine/conftest.py): one
``alembic upgrade head`` per session (now through migration 0005), per-test SCRAM
provisioning of ``delta_app``, and a privileged TRUNCATE before each test that resets
every tenant-scoped table this suite can touch (including budget_definitions, since
approval materializes rows there).
"""
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.change_history, delta.allocation_targets, delta.allocations, "
                    "delta.budget_publish_outbox, delta.budget_enforcement_state, "
                    "delta.budget_definitions CASCADE"
                )
            )
    finally:
