"""Fixtures for the D-024 micro-transaction execution DB suite. Mirrors
...
D-024 owns ``micro_transaction_executions`` (migration 0016) and writes into D-021's
``personal_transactions``/reads ``personal_accounts`` — all three are truncated
between tests.
"""
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.micro_transaction_executions, "
