"""Fixtures for the D-013 unified CRM DB suite. Mirrors ``tests/chargeback/conftest.py``
...
Unlike D-012 (pure read aggregates over the pre-existing D-003 ledger), D-013 owns its
own tables (migration 0007) — there is no lower-level "ingest" primitive to seed
through the way D-004's ``post_usage`` seeds the ledger. ``delta.crm.store`` (called
directly, then committed) IS the real, only application path for writing a client/
deal/interaction/stakeholder row, so seeding through it is the real path, not a
hand-inserted row.
"""
...
async def _truncate(provision_app_role: None) -> AsyncIterator[None]:
    ...
    try:
        async with engine.begin() as conn:
            await conn.execute(
                text(
                    "TRUNCATE delta.interactions, delta.stakeholders, delta.deals, "
                    "delta.clients CASCADE"
                )
            )
    finally:
...
@pytest.fixture
def seed_client():
    """Create a client via the real ``delta.crm.store`` path (the only application
    path for writing a client row — there is no lower-level ingest primitive to seed
    through, unlike D-012's ledger-backed reads)."""

    async def _seed(*, tenant_id: str, name: str = "Acme Corp", now: datetime) -> str:
