...
async def _read_history(tenant_id: str, *, entity_id: str | None = None) -> list[dict]:
    ...
    async with open_tenant_session(tenant_id) as s:
        rows = await list_history(s, entity_type="kill_switch_enforcement", entity_id=entity_id)
        return [
...
def read_history() -> Callable[..., Awaitable[list[dict]]]:
    """D-009: change_history rows for entity_type='kill_switch_enforcement', oldest first."""
