"""Dockerfile threat-model tests (D-010).
...
Anoryx-AI-Orchestrator's tests/deploy/test_dockerfile.py (O-008, ADR-0008),
itself mirroring Anoryx-Sentinel's tests/deploy/test_dockerfile.py (F-010,
ADR-0012 §9).
...
# tests/deploy/test_dockerfile.py -> Delta/
_DELTA_ROOT = Path(__file__).resolve().parents[2]
_DOCKERFILE = _DELTA_ROOT / "Dockerfile"
_DOCKERIGNORE = _DELTA_ROOT / ".dockerignore"
...
def test_dockerfile_declares_no_default_serve_command():
    # Delta has TWO ASGI apps sharing one image (delta.ingest.app:create_app /
    # delta.allocation_admin.app:create_app) — unlike the Orchestrator's single-
    # service image, this Dockerfile intentionally bakes NO default CMD; the
    # Helm Deployment / compose service for each app supplies its own explicit
    # command/args, and the migration Job supplies `alembic current`.
    text = _dockerfile_text()
    assert "CMD [" not in text, "the image must not bake a default CMD (two serve targets)"
