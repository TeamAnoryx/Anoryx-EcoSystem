...
- **Supersedes:** nothing. Adds a new `delta.bank_import` package, one new migration (0017:
...
| PAN/IBAN leakage via statement descriptors | 12+-digit-run rejection on `merchant`/`description`/`institution_label` → 422; charset-constrained `external_reference`; no raw-payload column exists (Fork 4) | `test_card_number_like_text_rejected`, code review — no JSONB/payload column in migration 0017 |
