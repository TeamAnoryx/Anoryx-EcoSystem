...
| **3 — micro-investment = a FIXED `MICRO_INVESTMENT_SURPLUS_RATE_BPS` (1000 bps = 10.00%) of net window surplus via EXACT INTEGER arithmetic, floored to exactly 0 when surplus <= 0** | `recommended_micro_investment_minor_units = 0 if surplus <= 0 else (surplus_minor_units * 1000) // 10_000`. Never negative, never a fabricated positive figure when the tenant is running a deficit, never off by even one minor unit at any magnitude. | Mirrors D-021's own honesty rule (`_savings_points`/`_budget_points`: an absent or unfavorable signal scores as exactly that, never reweighted to look favorable). A tenant with a negative surplus gets an honest "invest nothing right now" signal (0), not a nonsensical negative investment or a silently-clamped positive one. The rate is expressed as an exact integer basis-points numerator/denominator, never a `float` — the first implementation used `int(surplus_minor_units * 0.10)`, which both the code-reviewer and security-auditor independently flagged (Low/Medium) as violating `money.py`'s explicit "floats are forbidden in every monetary field" rule: `int(9_000_000_000_000_009 * 0.10) == 900_000_000_000_001`, one minor unit ABOVE the true floor `9_000_000_000_000_009 // 10 == 900_000_000_000_000`, at surplus magnitudes IEEE-754 cannot represent exactly. Fixed before merge; `test_large_surplus_micro_investment_uses_exact_integer_math` is the regression test proving the corrected formula is exact, not just "close enough," at that same magnitude. |
...
| Micro-investment recommendation rounds UP and over-recommends, incl. at surplus magnitudes where a float-rate multiplication is not exact under IEEE-754 (security-review finding, Fork 3) | `(surplus_minor_units * MICRO_INVESTMENT_SURPLUS_RATE_BPS) // 10_000` is EXACT integer arithmetic — `//` floors a nonnegative numerator at any magnitude, no float involved anywhere in the computation | `test_micro_investment_floors_toward_zero_never_overrecommends`, `test_large_surplus_micro_investment_uses_exact_integer_math` |
| Surplus silently computed ACCOUNT-scoped instead of the intended TENANT-wide figure (Fork 4) — the "obviously correct-looking" implementation to a future maintainer unaware of Fork 4's reasoning | `store.get_net_surplus_minor_units` has no `account_id` filter at all (queries every `personal_transactions` row visible to the tenant-scoped RLS session); a dedicated regression test seeds a SECOND account for the same tenant and proves its transaction is included in a recommendation computed against a DIFFERENT account (code-reviewer finding — no prior test could distinguish this from the rejected account-scoped alternative) | `test_surplus_computed_tenant_wide_across_all_accounts` |
