...
| **9 — currency-consistent budget adherence + validated list-window params (post-audit fix)** | The independent security-auditor review found two Low correctness bugs: (a) `get_latest_budgets` was not currency-scoped while the spend figures it was compared against were — a budget capped in EUR was silently scored as within-cap against a USD-scoped spend of 0, even when massively overspent, inflating the health score; (b) `GET /transactions`' `start`/`end` params were never validated for timezone-awareness or ordering, so a naive datetime hit the `timestamptz` comparison and either misread the window or 500'd. Fixed by (a) adding a `currency` parameter to `get_latest_budgets` — the health path passes the report currency (a non-report-currency budget is EXCLUDED from adherence, never scored as within-cap), while the plain list endpoint stays unscoped since each row carries its own currency label; and (b) mirroring the health route's `require_aware_utc` + `end > start` validation on the transactions list route (422 at the boundary). | Both findings are the same figure-honesty class D-020's audit caught (Fork 8 there): a favorable-looking number produced by silently mismatched data scoping. The health score is exactly the kind of authoritative-looking figure this monorepo's honesty mandate exists for. |
...
- Independent security-auditor review: verdict **CLEAN** of High/Critical findings.
  Two Low correctness findings (the currency-mismatched budget adherence and the
  unvalidated transactions-list window params, Fork 9), both fixed before merge with
  four new regression tests (36 total in `tests/personal_finance/`) — full details in
