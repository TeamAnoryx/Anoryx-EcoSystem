...
| Unbounded `GET /v1/admin/forecast/budgets` resource use | `definitions.list_budgets` capped at `MAX_LIST_LIMIT = 100` (mirrors D-007/D-008's own list caps) | code review — no independent test needed, same pattern as prior audited caps |
