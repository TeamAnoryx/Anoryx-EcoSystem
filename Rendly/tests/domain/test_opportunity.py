...
def test_opportunity_accepts_exactly_max_required_skills():
    poster = _profile("11111111-1111-4111-8111-111111111111")
    opp = _opportunity(poster, required_skills=tuple(f"s{i}" for i in range(MAX_REQUIRED_SKILLS)))
    assert len(opp.required_skills) == MAX_REQUIRED_SKILLS
...
def test_rank_opportunities_clamps_a_large_matching_pool_to_max_matches():
    poster = _profile("11111111-1111-4111-8111-111111111111")
    subject = _profile("22222222-2222-4222-8222-222222222222")
    subject_intent = _intent(subject, offering=("python",))
    opportunities = [
        _opportunity(poster, required_skills=("python",)) for _ in range(MAX_MATCHES + 1)
    ]

    ranked = rank_opportunities(subject, subject_intent, opportunities, limit=MAX_MATCHES + 1000)
    assert len(ranked) == MAX_MATCHES
