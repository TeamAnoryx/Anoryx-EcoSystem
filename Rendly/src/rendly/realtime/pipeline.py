...
THE SEND PIPELINE (FORK D — the seam is strictly BEFORE persist + fan-out):
  ...
  4. on PASS only: persist (assign message_id + per-channel seq, with the R-008 per-category
     ...
     trail for a send that ``messages`` structurally cannot show (ADR-0008). X-004 additionally
     fires a best-effort, metadata-only cross-product safety-event notification to the
     Anoryx-AI-Orchestrator (``safety_event_emitter.emit_block_events_best_effort``) on a genuine
     detector ``blocked`` outcome ONLY — fire-and-forget, never awaited, never able to delay or
     change this ack (ADR-0026).
...
from .safety_event_emitter import emit_block_events_best_effort
...
async def handle_chat_send(conn: Connection, data: dict, ctx: RuntimeContext) -> None:
    ...
    try:
        ...
        # No X-004 emission here: seam_unavailable has no detector category, and the wire
        # contract's outcome enum accepts only "block" (a genuine detector finding) — see
        # safety_event_emitter.py.
    ...
    if outcome.status == "blocked":
        ...
        # X-004 (ADR-0026): best-effort, metadata-only cross-product oversight notification.
        # Synchronous call — schedules fire-and-forget task(s) and returns immediately; cannot
        # delay or change the chat.ack below, and is a safe no-op if unconfigured.
        emit_block_events_best_effort(
            tenant_id=tenant_id,
            channel_id=channel_id,
            audit_id=audit_id,
...
async def _record_inspection_audit(
    *,
    ...
    ``audit_id`` is generated by the CALLER (not here, as of X-004) so it can also be used —
    on the ``blocked`` branch only — as the base of the ``idempotency_key`` handed to
    ``safety_event_emitter.emit_block_events_best_effort``, keeping the local audit row and the
    (optional) cross-product oversight notification correlated by the same id.
    """
    try:
        async with get_tenant_session(tenant_id) as session:
