...
async def test_migrations_reversible():
    """Vector 11: 0025/0026/0027 round-trip (head -> -3 -> head) is clean."""
    ...
    assert "0027" in (cur.stdout + cur.stderr), "expected head at 0027 before downgrade"

    # Downgrade all three F-019 migrations (0027 -> 0026 -> 0025 -> 0024).
    down = _run_alembic("downgrade", "-3")
    assert down.returncode == 0, f"downgrade -3 from 0027 failed:\n{down.stderr}"
    cur_down = _run_alembic("current")
    assert "0024" in (cur_down.stdout + cur_down.stderr), "expected 0024 after downgrade -3"
    ...
    assert "0027" in (cur_up.stdout + cur_up.stderr), "expected head at 0027 after re-upgrade"
