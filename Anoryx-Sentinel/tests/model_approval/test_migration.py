...
async def test_migrations_reversible():
    ...
    0031, ADR-0025 0032, F-026 0033, F-028 0034, F-033 0035). To stay correct
    regardless of the current head, we go to head, then downgrade to the EXPLICIT
    revision 0024 (pre-F-019) rather than a step count, then re-upgrade to head —
    verifying all F-019 migrations reverse.
    """
    ...
    assert "0035" in (cur.stdout + cur.stderr), "expected head at 0035 before downgrade"
    ...
    assert "0035" in (cur_up.stdout + cur_up.stderr), "expected head at 0035 after re-upgrade"
