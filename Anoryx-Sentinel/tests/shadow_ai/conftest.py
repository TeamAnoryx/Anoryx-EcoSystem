"""DB readiness for the shadow_ai test package (F-018).

Self-provisioning, mirroring tests/data_lock/conftest.py exactly:
this package provisions the schema (alembic upgrade head) + the sentinel_app
password before its DB-gated tests run, so it works on a fresh CI DB and when
run in isolation (memory: a new DB-test package needs its own DB-gated
provisioning).

DB-GATED: a no-op when DATABASE_URL/APP_DATABASE_URL are absent or Postgres is
unreachable, so the pure-unit shadow_ai tests run without a database.
...
async def _reset_db_engine_caches() -> AsyncIterator[None]:
    ...
    tests/data_lock/conftest.py + bulk/compliance/admin).
    """
    yield
...
async def _ensure_shadow_ai_db_ready() -> AsyncIterator[None]:
    ...
    if result.returncode != 0:
        pytest.fail(f"_ensure_shadow_ai_db_ready: alembic upgrade head failed:\n{result.stderr}")
