...
def test_current_head_is_0032() -> None:
    """Alembic current should report head at revision 0032 (ADR-0025 classifier thresholds)."""
    result = _run_alembic("current")
    assert result.returncode == 0, f"alembic current failed:\n{result.stderr}"
    assert "0032" in result.stdout or "0032" in result.stderr
...
def test_migration_downgrade_and_reapply() -> None:
    ...
    assert "0032" in result_current.stdout or "0032" in result_current.stderr
...
def test_incremental_downgrade() -> None:
    ...
    """
    num_revisions = 32  # 0001 through 0032
