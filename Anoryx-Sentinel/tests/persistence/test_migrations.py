...
def test_current_head_is_0033() -> None:
    """Alembic current should report head at revision 0033 (F-026 MCP server allow-list)."""
    result = _run_alembic("current")
    assert result.returncode == 0, f"alembic current failed:\n{result.stderr}"
    assert "0033" in result.stdout or "0033" in result.stderr
...
def test_migration_downgrade_and_reapply() -> None:
    ...
    assert "0033" in result_current.stdout or "0033" in result_current.stderr
...
def test_incremental_downgrade() -> None:
    ...
    """
    num_revisions = 33  # 0001 through 0033
