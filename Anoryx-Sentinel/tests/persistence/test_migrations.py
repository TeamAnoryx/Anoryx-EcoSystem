...
def test_current_head_is_0012() -> None:
    """Alembic current should report head at revision 0012 (F-011)."""
    result = _run_alembic("current")
    assert result.returncode == 0, f"alembic current failed:\n{result.stderr}"
    assert "0012" in result.stdout or "0012" in result.stderr
...
def test_migration_downgrade_and_reapply() -> None:
    ...
    assert "0012" in result_current.stdout or "0012" in result_current.stderr
...
def test_incremental_downgrade() -> None:
    ...
    """
    num_revisions = 12  # 0001 through 0012
