...
- [x] R-022 Mentorship matching by tech-stack 🏦 ✅ (shipped PR #119, scoped to a deterministic exact-tag scorer over a new MentorshipProfile opt-in type — tag/level pairs, matching requires an exact shared tag with differing proficiency levels — no ML, no persistence, no REST/UI; see ADR-0022)
