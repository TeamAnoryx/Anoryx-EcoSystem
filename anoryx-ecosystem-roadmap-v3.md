...
- [ ] F-035 External Pen-Test Pass 🔮 (external) — NOT auto-shippable: requires a HUMAN-INITIATED external third-party engagement; no agent/CI can perform or "pass" it. Engagement-ready package shipped at Anoryx-Sentinel/docs/pentest/ (scope, readiness checklist anchored on sentinel-preflight/F-031, remediation tracker). Stays unchecked until a real external test is completed and its Critical/High findings are remediated + retested.
- [x] F-036 Self-Hosted / Air-Gapped Deployment 🔮 (shipped — offline validation/integrity toolkit: OFFLINE license validation (ES256 signed claims reusing F-008 policy.crypto — no phone-home, fail-closed on bad sig/expiry/missing pubkey), signed install-bundle integrity (SHA-256 per artifact + single-signature manifest, verify_bundle recomputes + checks), mirror-config lint rejecting any public-internet host. sentinel-airgap CLI. Honest scope: validation toolkit NOT the release wheelhouse/registry-mirror pipeline or runtime license enforcement — deferred to docs/followups/f-036-release-bundle-build.md. See docs/adr/0041)
...
- [x] R-029 Project/sprint workspaces + B2B analytics 🏦 ✅ (shipped PR #138, scoped to a sprint-scheduling + permission-gated reporting seam reusing R-006's Channel as the workspace container + R-027's VIEW_TENANT_AUDIT_LOG permission — no new persisted Workspace entity, no task/issue tracking, no BI analytics engine, no new permission, no persistence, no REST/UI; see ADR-0029)
- [x] R-030 Public embedding API + developer portal 🏦 (shipped PR #141, scoped to a permission-gated, time-bounded `EmbedGrant` + public-safe `EmbedManifest` projection seam over R-013's Event/EventSession agenda — no developer portal, no REST/wire surface, no persisted API-key system, no embedding of Channel content; see ADR-0030)
...
- [x] X-003 Budget enforcement loop (killer feature) — Tricky (10-14h) ✅ (human security sign-off given 2026-07-11 to reverse ADR-0009 §11 R1 for `intake_policy()` only — ADR-0042; api-architect applied `POST /admin/policies/intake` to the contract; Sentinel mounts the thin-wrapper route — break-glass-only auth (require_admin + reject_sso_global), raw-bytes-preserving, maps every IntakeResult to its ADR-0042 status; O-004's distribution e2e now drives the REAL mounted route + real auth (hand-rolled shim retired), with assertions that only pass against the real route. Honest scope: the O-004→Sentinel hop is proven non-stubbed here; the Delta D-005→O-004 hop stays proven by Delta's own `test_o004_e2e.py` — driving all three from one live Delta cap-breach left a documented gap. See Sentinel ADR-0042.)
