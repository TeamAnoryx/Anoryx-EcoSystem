...
- [x] D-025 B2C privacy-first multi-bank aggregation 🏦 ✅ (shipped PR #148 — generic consent-scoped ingestion framework mirroring D-019's precedent: masked last-4 account reference only (DB-CHECK-enforced, no credential/token storage ever), consent-gated `linked_institutions` + append-only `aggregation_sync_runs`/`aggregation_ingested_references` (composite-PK dedup backstop), ingested transactions land in D-021's own `personal_transactions` ledger (source='aggregated'); no live Plaid/bank-OAuth integration exists in this codebase, see docs/adr/0025-delta-bank-aggregation.md)
