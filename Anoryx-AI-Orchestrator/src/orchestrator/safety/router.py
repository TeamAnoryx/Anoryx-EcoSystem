"""POST + GET /v1/safety/events — cross-product safety-event visibility (X-004).

Lets Sentinel, Delta, and Rendly each push a NORMALIZED "a LOCAL safety inspection
produced a non-pass outcome" record after their OWN in-product content inspection fires
— e.g. Rendly's R-008 self-hosted PII/injection/secret message inspector — into one
durably audited, tenant-queryable log. Mirrors O-010's identity-event correlation seam
exactly (same shape: product pushes a normalized event in, server-resolves
source_product from auth, idempotent on a dedup key, hash-chain audited, tenant-scoped
read).
...
Gated by a per-source-product bearer (`ORCH_SAFETY_SOURCE_TOKENS`) DISTINCT from every
other Orchestrator principal, including `identitySourceBearer` — source_product is
resolved FROM the matched token, never accepted from the request body. The read seam
reuses the EXISTING O-006 per-tenant principal (`query_service_tokens` /
`require_tenant_principal`) — the SAME credential that already gates `/v1/events` and
`/v1/identity/events` — rather than inventing a new tenant-read trust root.
"""
...
from orchestrator.config import SafetySettings
...
_KNOWN_CATEGORIES = frozenset({"pii", "injection", "secret"})
_KNOWN_OUTCOMES = frozenset({"block"})
...
def _resolve_source(request: Request, settings: SafetySettings) -> str | None:
    """Constant-time-match the presented bearer against every configured safety source
...
def _safety_event_body(row: dict[str, Any]) -> dict[str, Any]:
    body = {
...
async def ingest_safety_event(request: Request) -> JSONResponse:
    """Ingest one normalized cross-product safety-inspection outcome from Sentinel, Delta,
    or Rendly.
    ...
    """
    settings: SafetySettings = request.app.state.safety_settings
    request_id = _request_id()

    source_product = _resolve_source(request, settings)
    if source_product is None:
        return _error(401, "unauthorized", "safety source authentication required", request_id)
    ...
    if category not in _KNOWN_CATEGORIES:
        return _error(
            422, "schema_invalid", "category must be a known finding category", request_id
        )
    if outcome not in _KNOWN_OUTCOMES:
        return _error(422, "schema_invalid", "outcome must be a known non-pass outcome", request_id)
    ...
    async with get_tenant_session(tenant_id) as session:
        inserted = await insert_safety_event(session, row)
    ...
    async with get_privileged_session() as psession:
        async with psession.begin():
            await append_safety_audit_link(
                psession,
                tenant_id=tenant_id,
                source_product=source_product,
                category=category,
...
async def query_safety_events(
    ...
    """Cursor-paginated read of this tenant's cross-product safety events.
    ...
    filters = {"source_product": source_product, "category": category}
    async with get_tenant_session(principal) as session:
        rows, next_seq = await list_safety_events(
            session, filters=filters, limit=limit_value, cursor=decoded_cursor
        )
    body = {
        "data": [_safety_event_body(row) for row in rows],
